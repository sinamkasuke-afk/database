...
function getData() {
  ...
  if (!d) {
    localStorage.setItem('nuRequests', JSON.stringify(START));
    return START.map(x => [...x]);
  }
  return JSON.parse(d);
}
...
// Statistics and UI helper functions
function counts() {
  let d = getData();
  return {
    total: d.length,
...
function sidebar(active) {
  return `<aside class="sidebar">
    <div class="side-content">
      <div class="brand">
        <img src="assets/nu-logo.png" alt="NU Logo">
      ...
      <nav class="nav">
        ...
        <a class="${active == 'reservation' ? 'active' : ''}" href="reservation.html">
          <span class="icon">${ICONS.reservation}</span>Reservation Requests
        </a>
        <a class="${active == 'calendar' ? 'active' : ''}" href="calendar.html">
          <span class="icon">${ICONS.calendar}</span>Calendar
        </a>
        <a class="${active == 'history' ? 'active' : ''}" href="history.html">
          <span class="icon">${ICONS.history}</span>History
        </a>
        <a href="logout.html">
          <span class="icon">${ICONS.logout}</span>Logout
        </a>
      </nav>
      <img class="bulldog" src="assets/nu-logo.png" alt="NU Bulldog">
    </div>
    <div class="contact">
      <h3>PFMO Contact</h3>
